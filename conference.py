...
from datetime import datetime, time as time_func
...
from models import ConflictException, SessionForm, Session, SessionForms
...
class ConferenceApi(remote.Service):
    ...
    def getConferencesCreated(self, request):
        ...
        return ConferenceForms(
            items=[self._copyConferenceToForm(conf, getattr(prof, 'displayName')) for
                   conf in confs]
        )
    ...
    def _getQuery(self, request):
        ...
        for filtr in filters:
            ...
            formatted_query = ndb.query.FilterNode(filtr["field"], filtr["operator"],
                                                   filtr["value"])
    ...
    def _formatFilters(self, filters):
        ...
        for f in filters:
            ...
            if filtr["operator"] != "=":
                ...
                # disallow the filter if inequality was performed on a different field
                # before track the field on which the inequality operation is performed
    ...
    def queryConferences(self, request):
        ...
        return ConferenceForms(
            items=[self._copyConferenceToForm(conf, names[conf.organizerUserId]) for conf
                   in conferences]
        )
    ...
    def getConferenceSessions(self, request):
        ...
        if not c_key:
            raise endpoints.NotFoundException('No conference with key %s found.' % c_key)

        # get all sessions for conference with key c_key
        sessions = Session.query(ancestor=c_key)

        # copy session data objects into SessionForm objects for JSON serialization
        return SessionForms(
    ...
    def getConferenceSessionsByType(self, request):
        ...
        sessions = Session.query(
            Session.typeOfSession == request.typeOfSession, ancestor=c_key
        )
    ...
    @endpoints.method(message_types.VoidMessage, SessionForms,
                      path='getNonWorkshopsBefore7PM',
                      http_method='GET',
                      name='getNonWorkshopsBefore7PM')
    def getNonWorkshopsBefore7PM(self, request):
        """Gets non-workshops before 7:00pm"""
        sessions = Session.query(Session.time < time_func(hour=19))

        # filter out sessions where 'typeOfSession' field is 'workshop'
    ...
    def getFeaturedSpeaker(self, request):
        """Gets the featured speaker"""
        featured_speaker = memcache.get(MEMCACHE_FEATURED_SPEAKER_KEY)

        # raise exception is speaker is not set
        if not featured_speaker:
            raise endpoints.NotFoundException('No featured speaker is set.')
    ...
    def addSessionToWishlist(self, request):
        ...
        if not s_key:
            raise endpoints.NotFoundException('Session with key %s not found.' % s_key)

        if s_key.urlsafe() in prof.sessionKeysToAttend:
            raise endpoints.ConflictException(
                'You have already added this session to your wishlist'
            )

        prof.sessionKeysToAttend.append(s_key.urlsafe())
        prof.put()
    ...
    def getSessionsInWishlist(self, request):
        ...
        # get database keys from websafe/url keys
        sess_keys = [ndb.Key(urlsafe=websafeKey) for websafeKey in
                     prof.sessionKeysToAttend]

        # get sessions based on database keys
